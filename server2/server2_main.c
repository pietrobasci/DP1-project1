...
void sigchld_handler(int sig){
    ...
    //trace ( err_msg("(%s) - child %d terminated", prog_name, childpid) );
...
int main (int argc, char *argv[]) {
    ...
    while (1) {
        ...
        if((pid = fork()) < 0){
            ...
            /* child process */
            close(listenfd);    /* close passive socket */
            trace( err_msg ("(%s) child (pid=%d) created", prog_name, getpid()) );
            err = prot_a(connfd);   /* serve client */
            close (connfd);         //
