...
ssize_t readn_timeout (int fd, void *vptr, size_t n, int tm)
{
    size_t nleft;
    ssize_t nread;
    char *ptr;
    ...
    while (nleft > 0)
    {
        ...
        if ((m = select(FD_SETSIZE, &cset, NULL, NULL, &tval)) == -1)
...
ssize_t readline_unbuffered_timeout (int fd, void *vptr, size_t maxlen, int tm)
{
    int n, m, rc;
    char c, *ptr;
    int flg;
